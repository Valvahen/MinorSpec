...
# ceil((num_of_users_per_fog_node * NumOfTaskPerUser * NumOfFogNodes) / numOfTXperBlock)
...
user_informed = False
splitbft_mode = data['SplitBFT_mode']
...
def initiate_miners():
    ...
    if blockchainPlacement == 2:
        for i in range(NumOfMiners):
            the_miners_list.append(miner.Miner(i + 1, trans_delay, gossip_activated))
    print("Total miners created:", len(the_miners_list))
    print("Miner IDs:", [m.address for m in the_miners_list])
...
def initiate_genesis_block(AI_wanted, splitbft_mode):
    genesis_transactions = ["genesis_block"]
    for i in range(len(miner_list)):
        genesis_transactions.append(miner_list[i].address)
    genesis_block = new_consensus_module.generate_new_block(
        genesis_transactions, 'The Network', 0,
        type_of_consensus, AI_wanted, False,
        splitbft_mode=splitbft_mode
    )
    output.block_info(genesis_block, type_of_consensus)
    for elem in miner_list:
        elem.receive_new_block(
            genesis_block, type_of_consensus,
            miner_list, blockchainFunction, expected_chain_length
        )
...
def record_splitbft_statistics(splitbft_mode, elapsed_time, summary, expected_chain_length, filename='splitbft_results.csv'):
    """
    Append SplitBFT performance statistics to a CSV file.
    ...
    with open(filename, mode='a', newline='') as f:
        ...
        writer.writerow([
            ...
            summary.get("BlocksFinalized", ""),
            summary.get("Messages", ""),
            summary.get("TransactionsProcessed", ""),
            summary.get("TPS", "")
        ])
...
if __name__ == '__main__':
    ...
    splitbft_mode = sim_params.get("SplitBFT_mode", 0)

    print(f"\n\n===== Running SplitBFT Mode {splitbft_mode} from Sim_parameters.json =====\n\n")
    ...
    initiate_genesis_block(AI_assisted_mining_wanted, splitbft_mode=splitbft_mode)
    ...
    new_consensus_module.miners_trigger(
        miner_list, type_of_consensus, expected_chain_length,
        Parallel_PoW_mining, numOfTXperBlock, blockchainFunction,
        poet_block_time, Asymmetric_key_length,
    ...
    print(f"\n===== SplitBFT Efficiency Metrics (Mode: {splitbft_mode}) =====")
    for k, v in summary.items():
        print(f"{k}: {v}")
    record_splitbft_statistics(splitbft_mode, elapsed_time, summary,expected_chain_length)
    print("Data Saved")
