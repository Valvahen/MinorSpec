...
def test_quorum_size_never_below_minimum_floor(aqa_protocol):
    """
...
def test_quorum_rule_satisfies_agreement_condition():
    """Theorem 1: every quorum the protocol computes must satisfy 2Q - k > f."""
    for k in range(3, 101):
        for f in range(1, (k - 1) // 3 + 1):
            Q = min(max(2 * f + 1, (k + f) // 2 + 1), k)
            assert 2 * Q - k > f, (
                f"Quorum rule violates Theorem 1: k={k}, f={f}, Q={Q}, "
                f"2Q-k={2*Q-k}, need > {f}"
            )
